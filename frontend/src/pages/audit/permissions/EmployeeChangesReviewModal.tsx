...
interface ChangeGroup {
  ...
  inherited: { moduleKey: string; actionKey: string; actionName: string }[];
}

interface EmployeeChangesReviewModalProps {
  changes: EmployeePendingChange[];
  moduleDisplayName: string;
  employeeNames: Map<number, { name: string; code: string }>;
...
export function EmployeeChangesReviewModal({
  changes,
  moduleDisplayName,
  employeeNames,
  allActions,
  ...
  onCancel,
  isSaving,
}: EmployeeChangesReviewModalProps) {
  ...
  for (const c of changes) {
    if (!groups.has(c.employeeId)) {
      const info = employeeNames.get(c.employeeId) ?? { name: `Employee #${c.employeeId}`, code: '' };
      groups.set(c.employeeId, {
    ...
    const group = groups.get(c.employeeId)!;
    ...
    } else if (c.override === false) {
      group.denied.push({ moduleKey: c.moduleKey, actionKey: c.actionKey, actionName });
    } else {
      group.inherited.push({ moduleKey: c.moduleKey, actionKey: c.actionKey, actionName });
    }
  }
                  ...
                  {group.inherited.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 flex items-center gap-1.5">
                        ...
                        Reset to Inherit ({group.inherited.length})
                      </p>
                      <div className="flex flex-wrap gap-1.5">
                        {group.inherited.map((p) => (
                          <span
                            key={`inherit-${p.moduleKey}:${p.actionKey}`}
                            className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium bg-slate-100 text-slate-600 rounded-md border border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700"
                          >
        ...
        <div className="px-6 py-3 bg-amber-50 dark:bg-amber-900/10 border-t border-amber-200 dark:border-amber-800 shrink-0">
          <div className="flex items-start gap-2.5">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
              These changes will be applied immediately upon confirmation. Employee overrides take
              precedence over position permissions. Audit logging will record all changes with actor,
