...
export function CombinedFeedbackHistoryPage() {
  ...
  const generatePDF = async (item: CombinedHistoryItem) => {
    try {
      ...
      doc.text('360 FEEDBACK HISTORY REPORT', 105, 20, { align: 'center' });
      doc.setFontSize(10);
      ...
      doc.setTextColor(8, 85, 191);
      doc.setFont('helvetica', 'bold');
      doc.text('Evaluator Information', 20, 42);
      autoTable(doc, {
        startY: 47,
        body: [
      ...
      doc.text('Evaluatee Information', 20, nextY);
      autoTable(doc, {
        startY: nextY + 5,
        body: [
