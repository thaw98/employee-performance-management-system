        ...
        <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
            ...
            {(() => {
                const getCategoryStyle = (cat: string) => {
                    ...
                    if (c.includes('EXCELLENT') || c.includes('OUTSTANDING')) return 'from-[#2463eb] to-[#1d4ed8] text-white border-[#2463eb]/30';
                    if (c.includes('GOOD') || c.includes('ABOVE')) return 'from-[#3b82f6] to-[#2463eb] text-white border-[#60a5fa]/30';
            ...
            <div className="flex items-center justify-between">
                <button 
                    onClick={() => navigate(-1)}
                    className="group flex items-center gap-2 text-slate-400 hover:text-[#2463eb] transition-colors font-bold text-sm"
                >
                    <div className="w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center group-hover:bg-[#eff6ff] group-hover:border-[#bfdbfe] transition-all">
                        <ChevronLeft size={18} />
                    </div>
                    Back to List
                </button>
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="flex items-center gap-2 bg-gradient-to-r from-[#2463eb] to-[#1d4ed8] hover:from-[#1d4ed8] hover:to-[#1e40af] disabled:opacity-60 text-white px-4 py-2 rounded-xl font-bold text-xs shadow-md shadow-[#dbeafe] transition-all active:scale-95 disabled:scale-100 cursor-pointer disabled:cursor-not-allowed"
                    >
                        {isExporting ? (
                            <>
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 bg-[#eff6ff] text-[#1d4ed8] rounded-xl border border-[#dbeafe] text-[10px] font-black uppercase tracking-widest">
            ...
            <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden relative">
                <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-r from-[#2463eb] to-[#1d4ed8] opacity-5" />
                <div className="p-10 pt-16 relative">
                    <div className="flex flex-col md:flex-row items-center gap-8">
                        <div className="w-32 h-32 bg-[#eff6ff] rounded-[40px] border-4 border-white shadow-xl flex items-center justify-center text-[#2463eb] font-black text-4xl">
                            {empName.charAt(0)}
                        </div>
                        <div className="flex-1 text-center md:text-left space-y-2">
                            ...
                            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                                <div className="flex items-center gap-2 text-slate-500 font-bold text-sm">
                                    <Building2 size={16} className="text-[#2463eb]" /> {deptName}
                                </div>
                                <div className="flex items-center gap-2 text-slate-500 font-bold text-sm">
                                    <User size={16} className="text-[#2463eb]" /> {posName}
                                </div>
                                <div className="flex items-center gap-2 text-slate-500 font-bold text-sm">
                                    <Calendar size={16} className="text-[#2463eb]" /> {assignment.template?.name}
                                </div>
                            </div>
                        </div>
                        <div className="bg-slate-900 text-white p-8 rounded-[32px] text-center min-w-[200px] shadow-2xl shadow-[#2463eb]/20 relative group overflow-hidden">
                            <div className="absolute inset-0 bg-gradient-to-br from-[#2463eb]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                            <p className="text-[10px] font-black text-[#60a5fa] uppercase tracking-[0.2em] mb-1 relative z-10">Final Assessment</p>
                            ...
                            <div className={`mt-2 px-4 py-2 rounded-2xl text-[11px] font-black uppercase tracking-widest border shadow-lg flex items-center justify-center gap-2 relative z-10 bg-gradient-to-r ${
                                (assignment.ratingCategory || '').toUpperCase().includes('EXCELLENT') ? 'from-[#2463eb] to-[#1d4ed8] border-[#60a5fa]' :
                                (assignment.ratingCategory || '').toUpperCase().includes('GOOD') ? 'from-[#3b82f6] to-[#2463eb] border-[#60a5fa]' :
                                                    ...
                                                    return (
                                                        <div 
                                                            ...
                                                            className={`h-10 w-10 rounded-xl flex items-center justify-center text-xs font-black border-2 transition-all ${
                                                                isSelected 
                                                                ? 'bg-[#2463eb] text-white border-[#2463eb] shadow-lg shadow-[#2463eb]/30' 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-sm space-y-6">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-[#eff6ff] rounded-xl flex items-center justify-center text-[#2463eb]">
                            <ShieldCheck size={20} />
                        </div>
                ...
                <div className="bg-[#111827] p-10 rounded-[40px] border border-slate-800 shadow-2xl space-y-6">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-[#2463eb]/10 rounded-xl flex items-center justify-center text-[#60a5fa]">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-white/10">
                            <div className="flex flex-col">
                                ...
                                <span className="text-[9px] text-[#60a5fa]/60 font-medium">{assignment.hrSignedAt ? new Date(assignment.hrSignedAt).toLocaleDateString() : ''}</span>
                            </div>
                            <div className="h-14 w-40 bg-white/10 rounded-2xl p-2 flex items-center justify-center border border-white/10 shadow-inner">
                                {assignment.hrSignature ? (
                                    ...
                                    <span className="text-[#60a5fa]/40 font-bold text-xs uppercase tracking-tighter italic">Verified by HR</span>
                                )}
                            </div>
                        </div>
                    </div>
