...
import { Target, X, Save, AlertCircle, CheckCircle2, History, Calendar, ChevronDown, Briefcase, Filter } from 'lucide-react';
...
const KpiEditModal = ({ employee, onClose }: { employee: any, onClose: () => void }) => {
  ...
  useEffect(() => {
    if (kpis) {
      const filtered = kpis.filter(k => !isFuturePeriod(k.period));
      setEditedKpis(filtered.map(k => ({ ...k })));
    }
  }, [kpis]);
...
const KpiHistoryModal = ({
  ...
  const { data: historyData, isLoading } = useGetEmployeeKpiHistoryQuery({
  ...
  if (historyData) {
    historyData
      ...
      .forEach(item => {
        const monthYear = item.createdDate
          ? format(new Date(item.createdDate), 'MMMM yyyy')
          : 'Unknown Date';
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-100">
          <div>
            ...
            <p className="text-sm font-medium text-slate-500">
              Employee: <span className="font-bold text-slate-900">{employee.name}</span>
            </p>
          </div>
          <button
            onClick={onClose}
                  ...
                  {Object.entries(groups).map(([month, items]) => (
                    <React.Fragment key={month}>
                      ...
                      {items.map((kpi, idx) => (
                        <tr
                          key={kpi.id || idx}
                          className={`hover:bg-slate-50/50 transition-colors group ${kpi.recordStatus === 'Archived' ? 'opacity-60 grayscale-[0.2]' : ''}`}
                        >
                          ...
                          <td className="py-4 px-6 text-[10px] font-bold text-slate-400 uppercase">
                            ...
                            <div
                              className={`mt-1 px-2 py-0.5 rounded-full text-[9px] font-black uppercase border w-fit ${kpi.recordStatus === 'Active' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}
                            >
                              {kpi.recordStatus}
                            </div>
                          </td>
    ...
    <div className="space-y-8 animate-in fade-in duration-700 max-w-5xl mx-auto">
      ...
      <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="flex items-center gap-3 bg-slate-50 px-4 py-2.5 rounded-2xl border border-slate-200 shadow-sm">
