...
export const SelfAssessmentAssignmentsPage: React.FC = () => {
  ...
  const [viewingTemplate, setViewingTemplate] = useState<SelfAssessmentFormTemplateDto | null>(null);
  ...
  const assignedFormsByTemplateId = useMemo(() => {
    const map = new Map<number, FormListDto[]>();
  ...
  const getAssignedFormsForTemplate = (template: SelfAssessmentFormTemplateDto) =>
    (assignedFormsByTemplateId.get(template.id) ?? []).filter((form) => {
  ...
  const viewingTemplateForms = viewingTemplate ? getAssignedFormsForTemplate(viewingTemplate) : [];
    ...
    <div className="min-h-screen px-6 py-6 md:px-8 animate-fade-in">
      ...
      {viewingTemplate && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="assigned-employees-title"
        >
          <div className="w-full max-w-6xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900">
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-4 dark:border-slate-700">
              <div>
                <h3 id="assigned-employees-title" className="text-base font-bold text-slate-900 dark:text-white">
                  Assigned Employees
                </h3>
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                  {viewingTemplate.departmentName} / {viewingTemplate.positionName}
                </p>
              </div>
              <button
                type="button"
                onClick={() => setViewingTemplate(null)}
                className="inline-flex h-9 w-9 items-center justify-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200"
                aria-label="Close assigned employees modal"
              >
                <X size={18} />
              </button>
            </div>
            {viewingTemplateForms.length === 0 ? (
              <div className="px-5 py-12 text-center">
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">No current-cycle employees found</p>
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
              ...
              <div className="max-h-[70vh] overflow-auto">
                <table className="min-w-full text-xs">
                  <thead className="sticky top-0 bg-slate-50 text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:bg-slate-800 dark:text-slate-500">
                    <tr>
