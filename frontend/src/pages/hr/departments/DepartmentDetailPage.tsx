...
export default function DepartmentDetailPage() {
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      toast.error(error?.data?.message || 'Failed to remove position.')
    }
  }
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-600 px-6 pt-8 pb-16 mx-0">
        ...
        <div className="relative max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-2xl bg-white/15 backdrop-blur-sm flex items-center justify-center border border-white/20 shadow-lg flex-shrink-0">
              <Building2 size={24} className="text-white" />
            </div>
            <div>
              <Link to="/hr/departments" className="inline-flex items-center gap-1.5 text-blue-100 hover:text-white text-xs font-bold mb-2 transition-colors">
                <ArrowLeft size={14} />
                Back to Departments
              </Link>
              <div className="flex flex-wrap items-center gap-2">
                <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-white/15 border border-white/20 font-mono font-semibold text-white text-xs tracking-wide">
                ...
                {department && (
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${
                    ...
                    {isActive(department.status) ? <CheckCircle2 size={13} /> : <XCircle size={13} />}
                    {isActive(department.status) ? 'Active' : 'Inactive'}
                  </span>
                )}
              </div>
              <h1 className="text-2xl font-extrabold text-white tracking-tight mt-2">
                {isDepartmentLoading ? 'Loading department...' : department?.departmentName}
              </h1>
              <p className="text-blue-100 text-sm mt-0.5">Manage positions mapped to this department</p>
            </div>
          </div>
          <button
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-4">
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-4 py-3 flex flex-col sm:flex-row items-center gap-3 justify-between">
          ...
          <p className="text-xs font-medium text-slate-500 flex-shrink-0">
            {globalFilter ? <><span className="font-bold text-slate-700">{filteredCount}</span> result{filteredCount !== 1 ? 's' : ''} found</> : <><span className="font-bold text-slate-700">{positions.length}</span> positions total</>}
          </p>
        </div>
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                  <tr key={headerGroup.id} className="bg-slate-50 border-b border-slate-100">
                    {headerGroup.headers.map((header) => (
                      <th
                        key={header.id}
                        className={`px-5 py-3.5 text-left text-[11px] font-bold text-slate-500 uppercase tracking-widest transition-colors group ${header.column.getCanSort() ? 'cursor-pointer select-none hover:bg-slate-100/70' : ''}`}
              ...
              <tbody className="divide-y divide-slate-50">
                {isLoading ? (
                  ...
                  table.getRowModel().rows.map((row, idx) => (
                    <tr key={row.id} className={`transition-colors hover:bg-blue-50/30 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}`}>
                    ...
                    <td colSpan={columns.length} className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center gap-3 text-slate-400">
                        <div className="w-16 h-16 rounded-2xl bg-slate-100 flex items-center justify-center">
                          <BriefcaseBusiness size={32} className="text-slate-300" />
                        </div>
                        <div>
                          <p className="text-base font-bold text-slate-600">No positions found</p>
                          <p className="text-sm text-slate-400 mt-0.5">
                            {globalFilter ? 'Try a different search term.' : 'Add a position to this department to get started.'}
                          </p>
                        </div>
                      </div>
                    </td>
