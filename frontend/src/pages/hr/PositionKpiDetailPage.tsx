...
export const PositionKpiDetailPage: React.FC = () => {
  ...
  const { data: posResponse, isLoading: posLoading } = useGetPositionByIdQuery(Number(positionId), {
    skip: !positionId
  });

  const { data: kpis, isLoading: kpisLoading, refetch } = useGetPositionKpisQuery(
    { departmentId: Number(departmentId), positionId: Number(positionId), period: selectedPeriodLabel },
    { skip: !departmentId || !positionId }
  );

  const isLoading = deptLoading || posLoading || kpisLoading;
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase italic">Position KPI Detail</h1>
            <p className="text-slate-500 text-sm font-medium">Performance monitoring and KPI specifications for the position.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4 group hover:border-[#dbeafe] transition-all">
          ...
          <div className="min-w-0">
            ...
            <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight truncate">{position?.positionName || '...'}</h3>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter truncate">{department?.departmentName}</p>
          </div>
        </div>
            ...
            {kpis && kpis.length > 0 && (
              <tfoot>
                <tr className="bg-slate-50/50 border-t-2 border-slate-200">
                  <td colSpan={6} className="py-5 px-8 text-right text-xs font-black text-slate-900 uppercase tracking-widest">Position Overall Score</td>
...
interface PositionKpiEditModalProps {
  department: any;
  position: any;
  kpis: PositionKpi[];
  onClose: () => void;
...
const PositionKpiEditModal: React.FC<PositionKpiEditModalProps> = ({ department, position, kpis, onClose }) => {
  const [editedKpis, setEditedKpis] = useState<PositionKpi[]>([]);
  const [updateKpis, { isLoading: isUpdating }] = useUpdatePositionHrKpiActualsMutation();
  ...
  const handleChange = (index: number, field: keyof PositionKpi, value: any) => {
  ...
  const handleSave = async (status: 'DRAFT' | 'SUBMITTED') => {
    try {
      ...
      await updateKpis({ departmentId: department.departmentId, positionId: position.positionId, kpis: kpisWithStatus }).unwrap();
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden border border-slate-100">
        <div className="flex justify-between items-center p-8 border-b border-slate-100 bg-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">Update Position Actuals</h2>
              <p className="text-sm font-medium text-slate-500">Position: <span className="font-black text-slate-800">{position.positionName}</span></p>
            </div>
          </div>
