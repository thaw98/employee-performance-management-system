...
export const KpiHistoryPage: React.FC = () => {
                ...
                {items.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="py-4 px-6 text-center">
                      <span className="px-3 py-1 bg-blue-50 text-blue-600 text-sm font-black rounded-full">
                        {item.totalKpis}
                      </span>
                    </td>
  ...
  const renderRow = (item: any) => (
    <tr key={item.id} className={`hover:bg-slate-50/50 transition-colors group ${item.recordStatus === 'Archived' ? 'opacity-60 grayscale-[0.2]' : ''}`}>
      ...
      <td className="py-4 px-6">
        <span className="px-2 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black rounded-lg uppercase">
          {item.category}
        </span>
      </td>
      ...
      <td className="py-4 px-6 text-center text-sm font-black text-blue-600">{item.weight}%</td>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-xs font-black transition-all shadow-xl shadow-indigo-100 uppercase tracking-widest">
      ...
      <div className="flex bg-slate-100 p-1 rounded-2xl w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-black transition-all uppercase tracking-widest ${activeTab === 'all' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <LayoutGrid size={14} /> All History
        </button>
        <button
          onClick={() => setActiveTab('employee')}
          className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-black transition-all uppercase tracking-widest ${activeTab === 'employee' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <User size={14} /> By Employee
        </button>
        <button
          onClick={() => setActiveTab('department')}
          className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-black transition-all uppercase tracking-widest ${activeTab === 'department' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <Building2 size={14} /> By Department
        </button>
        <button
          onClick={() => setActiveTab('position')}
          className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-black transition-all uppercase tracking-widest ${activeTab === 'position' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
        >
                ...
                <div className="flex flex-col gap-2">
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-100 outline-none font-bold text-slate-800"
                    />
                  </div>
                  <select 
                    className="w-full px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-100"
