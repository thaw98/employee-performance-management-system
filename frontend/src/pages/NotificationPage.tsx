    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-[#dbeafe] text-[#2463eb] flex items-center justify-center">
            <BellRing size={22} />
          </div>
          <div>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-[#2463eb] text-white text-sm font-black disabled:opacity-40 disabled:cursor-not-allowed hover:bg-[#1d4ed8] transition-colors"
          >
            <CheckCheck size={18} />
            Mark all read
          </button>
      ...
      <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
            ...
            return (
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-black transition-colors border-b-2 ${
                  isActive
                    ? 'border-[#2463eb] text-[#1d4ed8] bg-white'
        ...
        <div className="px-6 py-4 border-b border-slate-100 bg-white flex items-center justify-between gap-3 flex-wrap">
          ...
          <select
            ...
            className="min-w-56 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 outline-none transition-colors focus:border-[#2463eb] focus:ring-2 focus:ring-[#dbeafe]"
          ...
          <div className={isFetching ? 'opacity-70 transition-opacity' : 'transition-opacity'}>
            {notifications.map((notification) => (
              <button
                ...
                className={`w-full text-left px-6 py-5 border-b border-slate-100 last:border-b-0 flex gap-4 transition-colors hover:bg-[#dbeafe]/70 ${
                  notification.read ? 'bg-white' : 'bg-[#dbeafe]/55'
                }`}
              >
                <span
                  className={`mt-2 w-2.5 h-2.5 rounded-full flex-shrink-0 ${
                    notification.read ? 'bg-transparent' : 'bg-[#2463eb]'
                  }`}
                />
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 flex-wrap">
          <button
            ...
            className="p-2 rounded-lg bg-white border border-slate-200 text-slate-600 disabled:opacity-30 disabled:cursor-not-allowed hover:border-[#bfdbfe] hover:text-[#1d4ed8] transition-colors"
          >
            <ChevronLeft size={20} />
          </button>
            ...
            item === 'ellipsis' ? (
              ...
              <button
                ...
                className={`min-w-10 h-10 px-3 rounded-lg text-sm font-black border transition-colors ${
                  item === page
                    ? 'bg-[#2463eb] border-[#2463eb] text-white'
                    : 'bg-white border-slate-200 text-slate-600 hover:border-[#bfdbfe] hover:text-[#1d4ed8]'
                }`}
              >
                {item + 1}
              </button>
