...
import { Save, Loader2, Clock, AlertTriangle, X, Calendar, ChevronRight } from 'lucide-react'
...
export function TimeSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await fetchCyclePreview()
      toast.success('Settings saved. Current duration is applied and future year type is queued when needed.')
    } catch (err: any) {
  ...
  if (!isHR) {
    return (
      <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500">
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm p-12 text-center">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
            Time settings configuration is available to HR users only.
          </p>
          <Link
    ...
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500">
      <div className="mb-8">
        <nav className="flex items-center gap-2 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">
          ...
          <span className="text-slate-600 dark:text-slate-300">Time Settings</span>
        </nav>
        <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Time Settings Configuration</h1>
        <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-400">
          Configure organization-wide review cycles, year type, and duration.
        </p>
      </div>

      <div className="space-y-6">
        {/* Global Time Configuration Section */}
