...
export type UpwardFeedbackStatus = 'OPEN' | 'CLOSED';
export type UpwardFeedbackEventType = 'CREATED' | 'REPLY_ADDED' | 'CLOSED' | 'NOTIFICATION_SENT';

export interface UpwardFeedbackReply {
  replyId: number;
  feedbackId: number;
  authorEmployeeId: number;
  authorEmployeeName: string;
...
export interface UpwardFeedbackHistory {
  historyId: number;
  feedbackId: number;
  actorEmployeeId: number;
  actorEmployeeName: string;
  eventType: UpwardFeedbackEventType;
  description: string;
  createdAt: string;
}
...
export interface UpwardFeedback {
  ...
  createdByUserId: number;
  updatedByUserId: number | null;
  replies?: UpwardFeedbackReply[];
  history?: UpwardFeedbackHistory[];
}

export interface UpwardFeedbackCreateRequest {
  message: string;
}

export interface UpwardFeedbackReplyRequest {
  message: string;
}
