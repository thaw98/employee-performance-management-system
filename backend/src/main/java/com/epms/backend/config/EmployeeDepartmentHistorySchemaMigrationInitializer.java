...
public class EmployeeDepartmentHistorySchemaMigrationInitializer implements BeanPostProcessor {
    ...
    private void migrate(DataSource dataSource) {
        ...
        jdbc.execute("""
                CREATE TABLE employee_department_history (
                    ...
                    CONSTRAINT fk_edh_from_dept FOREIGN KEY (from_department_id) REFERENCES `department`(department_id),
                    CONSTRAINT fk_edh_to_dept FOREIGN KEY (to_department_id) REFERENCES `department`(department_id),
                    CONSTRAINT fk_edh_from_pos FOREIGN KEY (from_position_id) REFERENCES `position`(position_id),
                    CONSTRAINT fk_edh_to_pos FOREIGN KEY (to_position_id) REFERENCES `position`(position_id)
    ...
    private void addMissingIndexes(JdbcTemplate jdbc) {
        if (!indexExists(jdbc, "employee_department_history", "idx_edh_employee_current")) {
            jdbc.execute(
                    "CREATE INDEX idx_edh_employee_current ON employee_department_history(employee_id, is_current)");
        }
        if (!indexExists(jdbc, "employee_department_history", "idx_edh_employee_start")) {
            jdbc.execute(
                    "CREATE INDEX idx_edh_employee_start ON employee_department_history(employee_id, effective_start_date)");
        }
