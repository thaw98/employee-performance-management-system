...
/**
 * Aligns legacy {@code users.employee_id} (numeric FK to {@code employees.id}) with JPA mapping
 * ({@code users.employee_id} VARCHAR referencing {@code employees.employee_id}). Must run before
 * Hibernate {@code ddl-auto=update} so ALTER COLUMN is not blocked by an incompatible FK.
 */
@Service
@RequiredArgsConstructor
public class UserEmployeeSchemaMigrationService {
	...
	public void migrateIfNeeded() throws Exception {
		...
		if (employeeIdColumnType == Types.BIGINT) {
			migrateNumericEmployeeIdColumnToEmployeeCode(jdbcTemplate);
		}

		ensureEmployeeForeignKeyToEmployeesEmployeeId(jdbcTemplate);
	}

	private void migrateNumericEmployeeIdColumnToEmployeeCode(JdbcTemplate jdbcTemplate) throws Exception {
		...
		dropUserEmployeeForeignKeys(jdbcTemplate);
	...
	private void ensureEmployeeForeignKeyToEmployeesEmployeeId(JdbcTemplate jdbcTemplate) throws Exception {
	...
	private void dropUserEmployeeForeignKeys(JdbcTemplate jdbcTemplate) throws Exception {
